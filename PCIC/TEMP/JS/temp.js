...
*/
async function temp(){
	let CFGtxt=await readfile('/PCIC/config.json');
	...
	//set CSS
	let headadd=await readfile("/PCIC/TEMP/html/head.html");
	...
	document.getElementById('header').innerHTML=await readfile(headURL);




	let bt=await readfile("/PCIC/TEMP/html/header2.html");
